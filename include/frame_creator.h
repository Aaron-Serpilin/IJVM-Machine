...
struct frame* frame_creator (struct frame *head_frame, int number_variables) {
    ...
    new_frame->local_variables = malloc(sizeof(word_t) * number_variables); //Each frame has its own variables array
    new_frame->main_stack = stack_creator(new_frame); //Initialization of each frame's stack
    new_frame->previous_program_counter = head_frame->main_stack->program_counter;
